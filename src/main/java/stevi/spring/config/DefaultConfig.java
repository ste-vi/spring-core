...
public class DefaultConfig implements Config {
     ...
     */
    @Override
    public <T> Class<? extends T> getImplementation(Class<T> interfaceType) { // todo: refactor this method
        Set<Class<? extends T>> types = reflectionsScanner.getSubTypesOf(interfaceType);
        if (types.size() > 1) {
            Set<Class<? extends T>> primaryImplementations = getImplementationsWithAnnotation(types, Primary.class);
